...
public class StrutDrawer extends ModWorldDrawer implements Drawable, Shaderable {
  ...
  private boolean checkDraw(StrutDrawData drawData, SegmentController segmentController) {
    return segmentController.getSegmentBuffer().existsPointUnsave(drawData.pieceAIndex)
        && segmentController.getSegmentBuffer().existsPointUnsave(drawData.pieceBIndex);
  }

  private StrutConnectorModule getModule(SegmentPiece segmentPiece) {
    return (StrutConnectorModule)
        Objects.requireNonNull(ServerUtils.getManagerContainer(segmentPiece.getSegmentController()))
            .getModMCModule(strutId);
  }
}
