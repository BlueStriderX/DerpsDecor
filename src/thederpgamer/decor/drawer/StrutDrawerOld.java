...
/**
 * Modified version of EnergyStreamDrawer for struts.
 *
 * @author TheDerpGamer
 * @since 09/01/2021
 */
public class StrutDrawerOld implements Drawable {

    private static final Int2ObjectOpenHashMap<TubeVerts> vMap = new Int2ObjectOpenHashMap<TubeVerts>();
    ...
    private final Vector3f sDir = new Vector3f();
    private final Vector3f eDir = new Vector3f();
    ...
    public static TubeVerts getVerts(int c) {
        TubeVerts tubeVerts = vMap.get(c);
        if(tubeVerts == null) {
            tubeVerts = new TubeVerts(c);
            vMap.put(c, tubeVerts);
        }
        return tubeVerts;
    }
    ...
    private static int getVertsFromRadius(float rr) {
        ...
        if(EngineSettings.USE_POLY_SCALING_ENERGY_STREAM.isOn()) {
            if(rr > 0.75) {
                if(rr < 1.5) maxVerts = 16;
                else if(rr < 6.0) maxVerts = 32;
                else maxVerts = 48;
            }
        }
        return maxVerts;
    ...
    private static void configMatrix(Matrix3f m, Vector3f dir) {
        ...
        if(forward.y > 0.999999) {
            ...
            if(right.lengthSquared() == 0) right.set(1, 0, 0);
            right.normalize();
        }
    ...
    public void buildMesh() {
        if(stabilizerPaths.size() == 0) return;
        ...
        for(StabilizerPath s : stabilizerPaths) {
            ...
            TubeVerts v = getVerts(getVertsFromRadius(s.getRadius()));
            ...
            while(s.nodes.containsKey(from)) {
                ...
                if(xD * xD + yD * yD + zD * zD > 1 || radius * radiusScale > 0.5f) {
                    ...
                    if(startPoint) {
                        ...
                        start.set(pMod - SegmentData.SEG_HALF + fromX, pMod - SegmentData.SEG_HALF + fromY, pMod - SegmentData.SEG_HALF + fromZ);

                        end.set(pMod - SegmentData.SEG_HALF + fromX + d.x, pMod - SegmentData.SEG_HALF + fromY + d.y, pMod - SegmentData.SEG_HALF + fromZ + d.z);
                        ...
                        actualVerts += v.putStraightSection(radiusInit, start, end, sDir, eDir, curStart, curStartPlus, s.graphicsTotalLength, buffer, normalBuffer, texCoordBuffer, max);
                    } else {
                    ...
                    if(startPoint) {
                        start.set(pMod - SegmentData.SEG_HALF + fromX + d.x, pMod - SegmentData.SEG_HALF + fromY + d.y, pMod - SegmentData.SEG_HALF + fromZ + d.z);

                        //end needed here only for dir
                        end.set(pMod - SegmentData.SEG_HALF + toX - d.x, pMod - SegmentData.SEG_HALF + toY - d.y, pMod - SegmentData.SEG_HALF + toZ - d.z);
                    ...
                    actualVerts += v.putStraightSection(radiusInit, start, end, sDir, eDir, realStart, realEnd, s.graphicsTotalLength, buffer, normalBuffer, texCoordBuffer, max);
                    sDir.set(eDir);

                    if(endPoint) {
                        start.set(pMod - SegmentData.SEG_HALF + toX - d.x, pMod - SegmentData.SEG_HALF + toY - d.y, pMod - SegmentData.SEG_HALF + toZ - d.z);

                        end.set(pMod - SegmentData.SEG_HALF + toX, pMod - SegmentData.SEG_HALF + toY, pMod - SegmentData.SEG_HALF + toZ);
                        ...
                        actualVerts += v.putStraightSection(radiusInit, start, end, sDir, eDir, curEndMinus, curEnd, s.graphicsTotalLength, buffer, normalBuffer, texCoordBuffer, max);
                    } else {
                        //this is a corner section
    ...
    private static class TubeVerts {
        ...
        public TubeVerts(int vertCount) {
        ...
        private int putStraightSection(final float radius, Vector3f start, Vector3f end, Vector3f startDir, Vector3f endDir, float tStart, float tEnd, float distTotal, FloatBuffer buffer, FloatBuffer normalBuffer, FloatBuffer texCoordBuffer, int maxIndex) {
            ...
            assert (maxIndex != 0); //Check if empty
            ...
            for(int i = 0; i < vertCount; i++) {
                ...
                if(i == vertCount - 1)
                    tubeTexCoord[(i + 1) % vertCount].y = 1; //Last texture coordinate must be 1 (loop around)
