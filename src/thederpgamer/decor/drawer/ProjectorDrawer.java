      ...
      for (Map.Entry<SegmentPiece, Object> entry : drawMap.entrySet()) {
        if (drawCount
            >= ConfigManager.getMainConfig()
                .getConfigurableInt("max-projector-draws-per-frame", 30)) return;
        SegmentPiece segmentPiece = entry.getKey();
        if (!segmentPiece
                .getSegmentController()
                .getSegmentBuffer()
                .existsPointUnsave(segmentPiece.getAbsoluteIndex())
            || segmentPiece
                    .getSegmentController()
                    .getSegmentBuffer()
            ...
            || !segmentPiece.getSegmentController().isFullyLoaded()) drawMap.remove(segmentPiece);
        else {
