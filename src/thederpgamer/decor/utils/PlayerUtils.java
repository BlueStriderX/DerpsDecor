...
public class PlayerUtils {
  ...
  public static void startConnectionRunner() {
    ...
    new StarRunnable() {
      @Override
      public void run() {
        if (connectingStrut == SECOND
            || !GameClient.getClientState().isInAnyStructureBuildMode()
            || PlayerUtils.getSelectedSlot().isEmpty()
            || !ElementKeyMap.getInfo(PlayerUtils.getSelectedSlot().getType())
                .getName()
                .toLowerCase()
                .contains("paint")) {
  ...
  public static InventorySlot getSelectedSlot() {
    return GameClient.getClientPlayerState()
        .getInventory()
        .getSlot(
  ...
  public static HashMap<PlayerState, Float> getPlayersInRange(
    ...
    for (PlayerState playerState : GameServer.getServerState().getPlayerStatesByName().values()) {
      ...
      float distance =
          Math.abs(
              Vector3fTools.distance(pos.x, pos.y, pos.z, playerPos.x, playerPos.y, playerPos.z));
