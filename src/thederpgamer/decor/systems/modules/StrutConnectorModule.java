...
public class StrutConnectorModule extends ModManagerContainerModule {
  ...
  public final ConcurrentHashMap<SegmentPiece[], StrutDrawData> blockMap =
  ...
  public void handle(Timer timer) {
    for (Map.Entry<SegmentPiece[], StrutDrawData> entry : blockMap.entrySet()) {
      if (GameCommon.isOnSinglePlayer()) { // Dumb network bullshit >:U
        if (entry.getKey()[0].getSegmentController().isFullyLoaded()
            && !getDrawMap().containsKey(entry.getKey()))
          getDrawMap().put(entry.getKey(), entry.getValue());
      } else if (GameCommon.isClientConnectedToServer()) {
        if (entry.getKey()[0].getSegmentController().isInClientRange()
            && !getDrawMap().containsKey(entry.getKey()))
          getDrawMap().put(entry.getKey(), entry.getValue());
      }
    }
  ...
  public void removeInvalidEntries() {
    for (Map.Entry<SegmentPiece[], StrutDrawData> entry : blockMap.entrySet()) {
      ...
      if (!segmentBuffer.existsPointUnsave(pieceA.getAbsoluteIndex())
          || segmentBuffer.getPointUnsave(pieceA.getAbsoluteIndex()).getType() != getBlockId()
          || !segmentBuffer.existsPointUnsave(pieceB.getAbsoluteIndex())
          || segmentBuffer.getPointUnsave(pieceB.getAbsoluteIndex()).getType() != getBlockId()) {
  ...
  public void updateToServer() {
    ...
    try {
      ...
      LogManager.logException(
          "Something went wrong while trying to send strut data to server", exception);
    }
  }
  ...
  public void onTagDeserialize(PacketReadBuffer packetReadBuffer) throws IOException {
    SegmentBufferInterface segmentBuffer =
        getManagerContainer().getSegmentController().getSegmentBuffer();
  ...
  public ArrayList<SegmentPiece> getConnections(SegmentPiece segmentPiece) {
    ...
    for (SegmentPiece[] segmentPieces : blockMap.keySet()) {
      if (segmentPieces[0].equals(segmentPiece) && !segmentPieces[1].equals(segmentPiece))
        connections.add(segmentPieces[1]);
      else if (segmentPieces[1].equals(segmentPiece) && !segmentPieces[0].equals(segmentPiece))
        connections.add(segmentPieces[0]);
    }
    return connections;
  }
}
