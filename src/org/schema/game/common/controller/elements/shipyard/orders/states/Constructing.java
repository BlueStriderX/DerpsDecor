...
import org.schema.game.common.controller.EntityCopyData;
...
public class Constructing extends ShipyardState{
    ...
    private List<Ship> ships = new ObjectArrayList<Ship>();
    ...
    public boolean onUpdate() throws FSMException {
        ...
        if(!checkedName){
            ...
            LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": Checking name exists: "+existsEntity);
            if(existsEntity){
        ...
        if(spawnedShip > 0){
            ...
            if(((GameServerState)getEntityState().getState()).getLocalAndRemoteObjectContainer().getLocalObjects().containsKey(lastSpawnedId) &&
               (System.currentTimeMillis() - spawnedShip > 5000 ||
                (getEntityState().getCurrentDocked() != null &&
                 getEntityState().getCurrentDocked().getId() == lastSpawnedId))){


                SegmentController spawned = (SegmentController)((GameServerState)getEntityState().getState()).getLocalAndRemoteObjectContainer().getLocalObjects().get(lastSpawnedId);
                ...
                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: waiting for construction DONE: CORE: "+pointUnsave);
                ...
                if(pointUnsave != null){
                    ...
                    if(!forceRequest) {
                        LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: waiting for construction Force request of 0 0 0: "+pointUnsave);

                        spawned.getSegmentProvider().enqueueHightPrio(0, 0, 0, true);
                        forceRequest = true;
                    }else {
                        LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: waiting for construction WAITING FOR SPAWN: "+pointUnsave);
                    }
                }
            }else{
                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: waiting for construction to be done");
            ...
            LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: ship spawned "+newShip+": creating docking relation: "+b);
            ...
            if(b){
                ...
                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: ship successfully spawned and docked");

            }else{
                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: unable to dock");
            ...
            currentDesign = null;
                 ...
                 && currentDesign != null
                 && currentDesign == getEntityState().getCurrentDesign()
                 && currentDocked != null && currentDocked.isVirtualBlueprint()
                 && currentDocked == getEntityState().getCurrentDocked()){

            LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: spawning new ship");

            if(getEntityState().spawnDesignWithoutBlocks ||
               getEntityState().currentMapFrom.equals(getEntityState().currentMapTo) || getEntityState().getShipyardCollectionManager().isInstantBuild()){

                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: spawning new ship: blocks all available");
                ...
                if(currentDocked.isFullyLoadedWithDock()){
                    ...
                    if (EntityRequest.existsIdentifierWOExc(getEntityState().getState(), fileName)) {
                        ...
                        if(pointUnsave != null){
                            ...
                            int factionId =
                                    getEntityState().getShipyardCollectionManager()
                                                    .isPublicException(getEntityState().lastOrderFactionId) ?
                                    getEntityState().lastOrderFactionId
                                                                                                            : getEntityState().getSegmentController().getFactionId();


                            newShip = ((Ship)currentDocked).copy(
                                    name,
                                    currentDocked.getSectorId(),
                                    tr,
                                    Lng.str("SHIPYARD_") + getEntityState().getSegmentController().getUniqueIdentifier(),
                                    factionId,
                                    ships, new EntityCopyData());
                ...
                if(currentDocked.isFullyLoadedWithDock()){
                    LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: spawning new ship: aquiring blocks start step: "+currentFill+" / "+currentDocked.getTotalElements()+" blocks");
                    ...
                    getEntityState().setCompletionOrderPercentAndSendIfChanged((double)currentFill / (double)currentDocked.getTotalElements());

                    LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: spawning new ship: aquiring blocks end step: "+currentFill+" / "+currentDocked.getTotalElements()+" blocks");
                }else {
                    LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction: spawning new ship: aquiring blocks ERROR: Design not fully loaded");
                }
            }else {
                LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": new ship exists, but yard is idle: "+newShip);
            }
        }else  if(newShip== null){
            LogUtil.sy().fine(getEntityState().getSegmentController()+" "+getEntityState()+" "+this.getClass().getSimpleName()+": construction interrupted. design no longer docked!");
                ...
                if(getEntityState().currentMapTo.get(type) < getEntityState().currentMapFrom.get(type)){
                    ...
                    if(have < goal){
                        ...
                        for(short s : ElementKeyMap.inventoryTypes){
                            LongOpenHashSet connectedInventoriesToThis = yard.getSegmentController()
                                                                             .getControlElementMap().getControllingMap().get(yard.getControllerElement().getAbsoluteIndex()).get(s);
