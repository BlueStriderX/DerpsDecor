...
public class ClearProjectorsCommand implements CommandInterface {
	...
	public boolean onCommand(PlayerState sender, String[] args) {
		...
		if(!(sendable instanceof SegmentController)) {
			if(sender.isControllingCore()) entity = (SegmentController) sender.getFirstControlledTransformableWOExc();
			else {
				PlayerUtils.sendMessage(sender, "You must either be piloting an entity or have one selected to use this command.");
				return true;
			}
		} else entity = (SegmentController) sendable;
		if(args == null || args.length == 0) if(entity != null) clearAllProjectors(entity);
		else return false;
		return true;
	}
	...
	private void clearAllProjectors(SegmentController entity) {
		...
		if(segmentController.getManagerContainer().getModMCModule(textId) != null && segmentController.getManagerContainer().getModMCModule(textId).getSize() > 0) {
			try {
				TextProjectorModule module = (TextProjectorModule) segmentController.getManagerContainer().getModMCModule(textId);
				module.resetAllProjectors();
			} catch(Exception exception) {
				exception.printStackTrace();
			}
