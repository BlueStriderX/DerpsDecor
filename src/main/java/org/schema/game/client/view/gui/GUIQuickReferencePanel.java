...
public class GUIQuickReferencePanel extends GUIElement implements GUIActiveInterface {
	...
	public void onInit() {
		...
		mainPanel.onInit();
		mainPanel.clearTabs();
		final String path = EngineSettings.LANGUAGE_PACK.getCurrentState().toString().toLowerCase(Locale.ENGLISH).startsWith("chinese") ? "infographics/power2.0/chinese/" : "infographics/power2.0/english/";
	...
	public GUIContentPane createDerpsDecorTab() {
		...
		{ // Projectors Tab
			...
			GUITexDrawableArea drawableArea = new GUITexDrawableArea(getState(), sprite.getMaterial().getTexture(), ((1024f - 800f) * 0.5f) / 1024f, 0);
			drawableArea.setWidth(800);
			GUIScrollablePanel scrollablePanel = new GUIScrollablePanel(10, 10, projectorsTab.getContent(0), getState());
